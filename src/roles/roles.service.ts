...
export class RolesService {
  ...
  async findOneRol(id: number): Promise<Role> {
    const rol = await this.roleRepository.findOne({where: {idRole:id, status: '1'}});
  ...
  async updateRol(id: number, updateRoleDto: UpdateRoleDto): Promise<Role> {
    const finRol = await this.roleRepository.find({where:{idRole:id, status: '1'}});
  ...
  async removeRol(id: number): Promise<String> {
    const roleToDelete = await this.roleRepository.find({where:{idRole: id, status: '1'}});
